...
import { saveCyclingWorkout } from "../../services/cyclingService";
...
const RideSummaryScreen: React.FC<RideSummaryScreenProps> = ({
  navigation,
  route,
}) => {
  ...
  const avgSpeedKmh = ((workout.averageSpeed || 0) * 3.6).toFixed(1);
  const maxSpeedKmh = ((workout.maxSpeed || 0) * 3.6).toFixed(1);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await saveCyclingWorkout(workoutWithUser);
      
      Alert.alert(
        "¡Guardado!", 
        "Tu ruta ha sido guardada correctamente.", 
      ...
      >
        ...
        <View style={styles.secondaryStats}>
          <View style={styles.statCard}>
            ...
            <Text style={styles.statValue}>{avgSpeedKmh}</Text>
            <Text style={styles.statLabel}>Vel. Promedio (km/h)</Text>
          </View>

          <View style={styles.statCard}>
            <MaterialIcons name="flash-on" size={24} color="#F44336" />
            <Text style={styles.statValue}>{maxSpeedKmh}</Text>
